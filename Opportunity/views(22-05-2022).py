...
def showOpp(objs):
    ...
    for obj in objs:
        ...
        if oppCardCode != "":
            oppCardCodeObj = BusinessPartner.objects.filter(CardCode = oppCardCode).values("CardCode", "CardName")
            oppCardCodeJson = BusinessPartnerSerializer(oppCardCodeObj, many = True)
            finalOppData['CardCode']=json.loads(json.dumps(oppCardCodeJson.data))
        else:
            finalOppData['CardCode'] = []
