...
def all_filter(request):
    ...
    PageNo = request.data['PageNo']     #### 10/02/2023 update
    try:
    ...
    if len(json_data) == 0:
        leads_obj = Lead.objects.filter(junk=0).order_by("-id")[startWith:endWith]
        ...
        if leadType =="None":
            leads_obj = Lead.objects.filter(assignedTo__in=SalesPersonID).order_by("-id")[startWith:endWith]
        elif leadType !="All":
            leads_obj = Lead.objects.filter(assignedTo__in=SalesPersonID, leadType=leadType,junk=0).order_by("-id")[startWith:endWith]
        else:
            leads_obj = Lead.objects.filter(assignedTo__in=SalesPersonID,junk=0).order_by("-id")[startWith:endWith]                       
