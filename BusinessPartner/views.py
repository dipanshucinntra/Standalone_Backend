...
def payment_summary(request):
    try:
        ...
        ttl = Order.objects.filter(CardCode=CardCode, CancelStatus = 'csNo').values_list('DocTotal', flat=True)
        print(ttl)
        total_sale=0
        for tt in ttl:
        ...
        pmt = Payment.objects.filter(CardCode=CardCode).values_list('ReceivedAmount', flat=True)
        print(pmt)
        total_pmt=0
        for pm in pmt:
        ...
        return Response({"message": "Success","status": 200,"data":[{"TotalPayment":total_sale, "ReceivedPayment":total_pmt, "PendingPayment":ttl_out}]})
        #return Response({"message": "Success","status": 200,"data":[]})
    except Exception as e:    
        return Response({"message": str(e),"status": 201,"data":[]})
