...
for campaign in allRow:
    ...
    if AllBPCheck == 1:
        ...
        BPCountryCode = campaign[27].strip()
        BPStateCode = campaign[28].strip()
    ...
    # delete campaign memeber by camp id
    sqlDeleteCampainMember = "DELETE FROM `Campaign_campaignsetmembers` WHERE `CampSetId_id` = "+str(camSetId)
    print(sqlDeleteCampainMember)
    mycursor.execute(sqlDeleteCampainMember)
    mydb.commit()
