...
def dashboard(request):
    ...
    if "SalesEmployeeCode" in json_data:
        ...
        if json_data['SalesEmployeeCode']!="":
            ...
            #{"SalesEmployeeCode":"2"}
            return Response({"message": "Success","status": 200,"data":[{"notification":notification, "amount":amount, "sale":sale, "sale_diff":sale_diff, "Opportunity":opp_all, "Quotation":quot_all, "Order":ord_all, "Customer":bp_all, "Leads":lead_all, "Over":ord_over, "Open":ord_open, "Close":ord_close, "campset_count" : campset_count}]})
...
#updated by millan on 16-11-2022 for showing top 5 customers based on sales person code and order
@api_view(["POST"])
def top5bp(request):
    try:
        SalesPersonCode = request.data['SalesPersonCode']
