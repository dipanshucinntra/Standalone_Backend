...
def login(request):
    try:
        ...
        if Employee.objects.filter(Q(userName=request.data['userName']) | Q(Email=request.data['userName']), Active = "tYES").exists():
            ...
            #res = root_login("root", "Sunil@123")            
            #return Response({"message": "Success","status": 200,"data":employee_json.data, "SAP":"data", "token":str(token)})
        else:
            return Response({"message": "InActive User Login Not Allowed", "status": 201,"data":[]})
    except Exception as e:
        return Response({"message": "Username or Password is incorrect","status": 404,"data":[{"err":str(e)}]})
...
def root_login(username, password):
    ...
    if not user:
        return Response({'status':201, 'error': 'Invalid Credentials'})
        #return Response({'error': 'Invalid Credentials'}, status=HTTP_404_NOT_FOUND)
    token, _ = Token.objects.get_or_create(user=user)
    
    return {'status':201, 'token': token.key}
    #return Response({'token': token.key}, status=HTTP_200_OK)
